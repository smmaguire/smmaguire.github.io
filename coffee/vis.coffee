...
Network = () ->
  ...
  update = () ->
    ...
    # radial layout
    if layout == "radial"
      sub_groups = sortedSub_groups(curNodesData, curLinksData)
      updateCenters(sub_groups)
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.sub_group))
  ...
  setupData = (data) ->
    ...
    countExtent = d3.extent(data.nodes, (d) -> d.din)
  ...
  filterNodes = (allNodes) ->
    ...
    if filter == "highly_connected" or filter == "weakly_connected"
      degree = allNodes.map((d) -> d.dout).sort(d3.ascending)
      cutoff = d3.quantile(degree, 0.5)
      filteredNodes = allNodes.filter (n) ->
        if filter == "highly_connected"
          n.dout > cutoff
        else if filter == "weakly_connected"
          n.dout <= cutoff

    filteredNodes

  # Returns array of sub_groups sorted based on
  # current sorting method.
  sortedSub_groups = (nodes,links) ->
    sub_groups = []
    if sort == "links"
      counts = {}
      links.forEach (l) ->
        counts[l.source.sub_group] ?= 0
        counts[l.source.sub_group] += 1
        counts[l.target.sub_group] ?= 0
        counts[l.target.sub_group] += 1
      # add any missing sub_groups that dont have any links
      nodes.forEach (n) ->
        counts[n.sub_group] ?= 0

      # sort based on counts
      sub_groups = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      # get just names
      sub_groups = sub_groups.map (v) -> v.key
    else
      # sort sub_groups by number of members
      counts = nodeCounts(nodes, "sub_group")
      sub_groups = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      sub_groups = sub_groups.map (v) -> v.key
  ...
  updateCenters = (sub_groups) ->
    if layout == "radial"
      groupCenters = RadialPlacement().center({"x":width/2, "y":height / 2 - 100})
        .radius(300).increment(18).keys(sub_groups)
  ...
  moveToRadialLayout = (alpha) ->
    k = alpha * 0.1
    (d) ->
      centerNode = groupCenters(d.sub_group)
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.sex)).darker().toString()

  # Mouseover tooltip function
  showDetails = (d,i) ->
    content = '<p class="main">' + d.id + '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + d.sub_group + '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + d.sex + '</span></p>'
...
$ ->
  ...
  $("#community_select").on "change", (e) ->
    communityFile = $(this).val()
    d3.json "data/#{communityFile}", (json) ->
  ...
  d3.json "data/groupA_day1.json", (json) ->
    myNetwork("#vis", json)
